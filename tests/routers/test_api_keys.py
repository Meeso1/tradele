...
def test_api_key_routes_require_authentication():
    assert client.post("/api/api-keys", json={"name": "key"}).status_code == 401
    assert client.get("/api/api-keys").status_code == 401
    assert client.post("/api/api-keys/some-id/deactivate").status_code == 401


def test_malformed_basic_credentials_return_401():
    garbage = base64.b64encode(b"not-base64-of-two-parts").decode()
    response = client.get("/api/api-keys", headers={"Authorization": f"Basic {garbage}"})

    assert response.status_code == 401


def test_unsupported_authorization_scheme_returns_401():
    response = client.get("/api/api-keys", headers={"Authorization": "Digest something"})
...
def test_api_key_routes_reject_regular_user_token(
    ...
    response = client.get("/api/api-keys", headers=auth_headers(user_id))
...
def test_api_key_routes_reject_service_account_token():
    ...
    response = client.get("/api/api-keys", headers={"Authorization": f"Bearer {token}"})

    assert response.status_code == 403


def test_create_and_use_api_key(service_auth_headers: dict[str, str]):
    response = client.post("/api/api-keys", json={"name": "worker key"}, headers=service_auth_headers)
    ...
    portfolio = client.get(
        "/api/portfolio", headers={"Authorization": body["authorization"]}
    )
    assert portfolio.status_code == 200


def test_list_api_keys_hides_key_values(service_auth_headers: dict[str, str]):
    client.post("/api/api-keys", json={"name": "key"}, headers=service_auth_headers)

    response = client.get("/api/api-keys", headers=service_auth_headers)
...
def test_deactivate_api_key(service_auth_headers: dict[str, str]):
    created = client.post(
        "/api/api-keys", json={"name": "key"}, headers=service_auth_headers
    ).json()
    response = client.post(
        f"/api/api-keys/{created['id']}/deactivate", headers=service_auth_headers
    )
    ...
    rejected = client.get(
        "/api/portfolio", headers={"Authorization": created["authorization"]}
    )
...
def test_deactivate_unknown_api_key_returns_404(service_auth_headers: dict[str, str]):
    response = client.post(
        "/api/api-keys/does-not-exist/deactivate", headers=service_auth_headers
    )
...
def test_deactivate_authenticating_key_returns_409(service_auth_headers: dict[str, str]):
    response = client.post(
        f"/api/api-keys/{_active_service_key_id()}/deactivate",
